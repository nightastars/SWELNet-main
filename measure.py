...
def normalize_(image, MIN_B=-300.0, MAX_B=300.0):
...
def compute_measure1(x, y, data_range):
    psnr = compute_PSNR(x, y, data_range)
    return psnr
def compute_measure2(x, y, data_range):
    ssim = compute_SSIM(x, y, data_range)
    return ssim
def compute_measure3(x, y):
    rmse = compute_RMSE(x, y)
    return rmse

def compute_MSE1(img1, img2):
    img1=normalize_(img1)
    img2=normalize_(img2)
...
def compute_RMSE(img1, img2):
    if type(img1) == torch.Tensor:
        return torch.sqrt(compute_MSE1(img1, img2)).item()
    else:
        return np.sqrt(compute_MSE1(img1, img2))
